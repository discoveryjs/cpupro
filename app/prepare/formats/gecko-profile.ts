...
import { UniformProfilingSession } from './types.js';
...
export function extractFromGeckoProfile(profile: GeckoProfile): UniformProfilingSession {
    const startTime = (profile.meta.startTime ?? 0) + (profile.meta.profilingStartTime ?? 0);
    const processes: UniformProfilingSession['processes'] = profile.threads.map(thread => ({
        pid: Number(thread.pid ?? -1),
        name: thread.processName ?? null
    }));
    ...
    return {
        ...
        threads: threads.sort((a, b) => a.pid - b.pid || a.tid - b.tid),
